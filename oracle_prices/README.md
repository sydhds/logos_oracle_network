...
```
oracle_prices/
├── oracle_prices_core/    # Shared types (used by guest + host)
│   └── src/lib.rs
├── oracle_prices_ffi/     # C FFI cdylib (compiled to .so for Qt)
...
│       └── src/bin/oracle_prices.rs
├── examples/             # CLI tools
│   └── src/bin/
│       ├── generate_idl.rs    # One-liner IDL generator
│       └── oracle_prices_cli.rs # Three-line CLI wrapper
├── spel.toml                         # SPEL CLI config (IDL and binary paths)
├── Makefile
└── oracle_prices-idl.json       # Auto-generated IDL
```
