...
```
oracle_register/
├── oracle_register_core/    # Shared types (used by guest + host)
│   └── src/lib.rs
├── oracle_register_ffi/     # C FFI cdylib (compiled to .so for Qt)
...
│       └── src/bin/oracle_register.rs
├── examples/             # CLI tools
│   └── src/bin/
│       ├── generate_idl.rs    # One-liner IDL generator
│       └── oracle_register_cli.rs # Three-line CLI wrapper
├── spel.toml                         # SPEL CLI config (IDL and binary paths)
├── Makefile
└── oracle_register-idl.json       # Auto-generated IDL
```
